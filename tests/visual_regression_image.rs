//! End-to-end visual regression for [`ImagePrinter`].
...
use winprint::{
    printer::{FilePrinter, ImagePrinter},
    ticket::{
...
fn image_visual_regression() {
    ...
    let actual_pages = print::print_pages(|device| {
        let origin = visual_regression_dir.join("data.tiff");
        let printer = ImagePrinter::new(device.clone());
